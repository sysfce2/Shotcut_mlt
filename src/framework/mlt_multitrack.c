...
void mlt_multitrack_refresh(mlt_multitrack self)
{
    ...
    for (i = 0; i < self->count; i++) {
        ...
        if (producer != NULL) {
            ...
            length = mlt_producer_get_playtime(producer) > length
...
int mlt_multitrack_connect(mlt_multitrack self, mlt_producer producer, int track)
{
    ...
    int result = mlt_service_connect_producer(MLT_MULTITRACK_SERVICE(self),
    ...
    if (result == 0) {
        ...
        self->list[track]->event = mlt_events_listen(MLT_PRODUCER_PROPERTIES(producer),
                                                     self,
                                                     "producer-changed",
                                                     (mlt_listener) mlt_multitrack_listener);
...
int mlt_multitrack_insert(mlt_multitrack self, mlt_producer producer, int track)
{
    ...
    // Connect to the producer to ourselves at the specified track
    int result = mlt_service_insert_producer(MLT_MULTITRACK_SERVICE(self),
                                             MLT_PRODUCER_SERVICE(producer),
                                             track);
        ...
        if (producer != NULL) {
            ...
            if (playlist != NULL) {
                for (j = 0; j < mlt_playlist_count(playlist); j++) {
                    count = add_unique(map,
                                       count,
                                       mlt_playlist_clip(playlist, mlt_whence_relative_start, j));
                    map = resize_set(map, count, &size);
                }
...
static int producer_get_frame(mlt_producer parent, mlt_frame_ptr frame, int index)
{
    ...
    if (index >= 0 && index < self->count && self->list[index] != NULL) {
        ...
        int hide = mlt_properties_get_int(MLT_PRODUCER_PROPERTIES(mlt_producer_cut_parent(producer)),
                                          "hide");
